...
export function stemToken(word) {
    ...
    if (w.length > 6 && w.endsWith('ings'))
        return w.slice(0, -4);
    if (w.length > 6 && w.endsWith('ing'))
        return w.slice(0, -3);
    if (w.length > 5 && w.endsWith('ers'))
        return w.slice(0, -3);
    if (w.length > 6 && w.endsWith('tions'))
        return w.slice(0, -5); // medications→medica (matches medication→medica)
    if (w.length > 5 && w.endsWith('ions'))
        return w.slice(0, -4);
    ...
    if (w.length > 5 && w.endsWith('ies'))
        return w.slice(0, -3); // allergies→allerg
    if (w.length > 6 && w.endsWith('ic'))
        return w.slice(0, -2); // allergic→allerg
    ...
    if (w.length > 3 && w.endsWith('s'))
        return w.slice(0, -1);
    if (w.length > 5 && w.endsWith('y'))
        return w.slice(0, -1); // allergy→allerg
    return w;
}
// ── Token alias expansion ─────────────────────────────────────────────────────
export function expandTokenAliases(stemmed) {
    const result = new Set([stemmed]);
...
export function normalizePhrases(text) {
